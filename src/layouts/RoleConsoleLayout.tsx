...
import { rolePageTitle } from '@/config/roleConsoleConfig';
...
export function RoleConsoleLayout({ config }: { config: RoleConsoleConfig }) {
  ...
  const [pageTitle, pageSubtitle] = useMemo(
    () => rolePageTitle(config, location.pathname),
    [config, location.pathname],
  );

  const isDashboard = location.pathname === config.dashboardPath;
  const isImmersive = config.immersivePathPatterns?.some((rx) => rx.test(location.pathname)) ?? false;
  const firstName = user?.name?.split(/\s+/)[0] ?? ROLE_LABELS[config.role];
  ...
  const title = isDashboard
    ? (config.welcomeTitle ? config.welcomeTitle(firstName) : pageTitle)
    : pageTitle;
  const subtitle = isDashboard
    ? (config.welcomeTitle ? pageSubtitle : todayLabel)
    : pageSubtitle;
  ...
  return (
    <div className="ct-shell">
        ...
        <div className="ct-topbar-brand-col">
          <div className="ct-topbar-brand">
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--ct-text-muted)', marginTop: 2 }}>{config.brandSubtitle}</div>
            </div>
          </div>
        </div>
      ...
      <div className="ct-body">
        <aside className="ct-sidebar">
          <div className="ct-sidebar-scroll ct-scroll">
            <GenericNavSections sections={config.navSections} />
          </div>
          <div className="ct-sidebar-footer">
            {switchOpen && !isMobile ? (
              <div className="ct-popover ct-scroll">
                <RoleSwitchPopover current={config.role} onClose={() => setSwitchOpen(false)} />
              </div>
            ) : null}
            {config.profilePath ? (
              <Link to={config.profilePath} className="ct-press ct-profile-card">
                <Avatar initials={user?.initials ?? 'CT'} tint="sage" size={34} />
                <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
                  <div className="ct-profile-card-label">Account</div>
            ...
            <div className="ct-sidebar-footer-actions">
              <button type="button" className="ct-press ct-switch-btn" onClick={() => setSwitchOpen((v) => !v)}>
                <IconSwitch size={14} />
                Switch view
              </button>
              <button
        ...
        <main className={`ct-main ct-scroll${config.mainSurfacePaths?.includes(location.pathname) ? ' ct-main-surface' : ''}`}>
      ...
      {switchOpen && isMobile ? (
        <RoleSwitchSheet current={config.role} onClose={() => setSwitchOpen(false)} />
      ) : null}

      <RoleMobileNav config={config} />
    </div>
  );
}
